...
To process the translation events and calculate the moving average delivery time, you can use the `unbabel_cli` application with either the example input file (`input_given.json`) provided in the exercise or the larger dataset generated by the script (`input_large.json`). Here are the commands for both scenarios:

#### Using the Example Input File
```bash
python3 unbabel_cli.py --input_file inputs/input_given.json --window_size 10 --output_file outputs/output.json
```
...
This commands processes the data in `inputs/input_given.json` and outputs the results to `outputs/output.json` on the first example and processes the data in `inputs/input_large.json` and outputs the results to `outputs/output_large.json`on the second one.
